...
const getFixedDeposit = asyncHandler(async (req, res) => {
  let amount = req.body.amount;
  let id = req.body.id;
  amount = parseInt(amount);
  const accnt = await Account.findOne({ accountholder: id });
  ...
  if (accnt) {
    if (accnt.totalamount >= amount) {
      ...
      res
        .status(500)
        .send(`Insufficient minimum balance needed for ${amount} Fixed Deposit`);
    }
  } else {
    res.status(500).send("Account Not found");
  }
...
module.exports = { createAccount, sendMoney, getLoan, getData, getFixedDeposit };
